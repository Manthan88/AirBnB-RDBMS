...
AvgListingRating NUMERIC(8,2), --Calc Field,
...
CREATE TABLE tblPayment(
    ...
    OriginalAmount NUMERIC(8,2), --calc field,
    TaxAmount NUMERIC(8,2), --calc field,
    FinalAmount NUMERIC(8,2), --calc field,
...
select city, stateid as state into tempcitystate from tblCity
...
SET @RUNAmenity = (Select floor(Rand()*(10-2+1))+1)
        ...
        While @RUNAmenity > 0
        BEGIN 
        SET @AmenityPK = (Select (Rand() * @Amenity_Count)+1)
...
SELECT S.State, S.StateID, C.City, SUM(P.FinalAMount) AS Revenue
FROM tblPayment P 
INNER JOIN tblBooking B ON P.BookingID = B.BookingID
INNER JOIN tblListing L ON B.ListingID = L.ListingID
INNER JOIN tblCity C ON L.CityID = C.CityID
INNER JOIN tblState S ON C.StateID = S.StateID
GROUP BY S.State, C.City, S.StateID

SELECT S.State, S.StateID, CT.CustomerType, Count(B.BookingID) Bookings
FROM tblCustomer C
INNER JOIN tblCustomerType CT ON C.CustomerTypeID = CT.CustomerTypeID
INNER JOIN tblBooking B ON C.CustomerID = B.CustomerID
INNER JOIN tblCity Ci ON C.CityID = Ci.CityID
INNER JOIN tblState S ON Ci.StateID = S.StateID
GROUP BY S.State, CT.CustomerType, S.StateID

Select S.State, S.StateID, concat(C.CustomerFname,' ', C.CustomerLname) as Customer_Name, B.CheckInDate, B.CheckOutDate, L.ListingName, avg(R.Rating) as Rating, R.RatingName
from tblState S
INNER JOIN tblCity CY on CY.StateID = S.StateID
INNER JOIN tblListing L on CY.CityID = L.CityID
INNER JOIN tblBooking B on L.ListingID = B.ListingID
INNER JOIN tblCustomer C on B.CustomerID =c.CustomerID
INNER JOIN tblReview RW on B.BookingID = RW.BookingID
INNER JOIN tblRating R on RW.RatingID = R.RatingID
GROUP by S.State, S.StateID, concat(C.CustomerFname,' ', C.CustomerLname), B.CheckInDate, B.CheckOutDate, L.ListingName, R.RatingName
...
SELECT x.StateID as Booking_State, ISNULL(y.Customer_Count,0) Cust_Count
FROM tblState x
LEFT JOIN 
(
SELECT d.StateID as Booking_State,  Count(e.CustomerID) Customer_Count
FROM tblBooking a 
INNER JOIN tblListing b ON a.ListingID = b.ListingID
...
WHERE g.StateID = 'WA'
GROUP BY d.StateID
)y ON x.StateID = y.Booking_State
ORDER BY Cust_Count